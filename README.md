...
- `0003_results_comment.sql` — adds a `comment` column to `results`, used by the result-feedback feature.
...
6. **Log your result** — tap *Registra risultato* and fill in the form for your score (weights/reps for strength, time for a for-time piece, rounds+reps for AMRAP, minutes completed for EMOM). Mark whether you did it RX and add notes if you scaled. If you'd generated recommendations first, Claude adds a short comment on how your result compares to the expected one.
