...
If you already have a database from before `fitness_profile` tracked history (one row per benchmark instead of one per dated result), also run [`supabase/migrations/0001_fitness_profile_history.sql`](supabase/migrations/0001_fitness_profile_history.sql) to migrate it in place.
